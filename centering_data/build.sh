...
IMG_NAME=$USER"/"${MAIN_NAME}"_"${ID}
CONTAINER_NAME=${MAIN_NAME}"_"${ID}
...
CRE_IMG="docker build -t "${IMG_NAME}" --build-arg proxy="${http_proxy}" --build-arg id="${ID}" --build-arg server="${SERVER}" "${CURRENT_DIR}
eval ${CRE_IMG}
